...
export default function AddAssetModal(props: Props) {
  ...
  // fetch masters saat modal dibuka
  useEffect(() => {
    if (!open) return
    let cancelled = false
    setIsLoading(true)
    ...
    ;(async () => {
      try {
        ...
        setDepartments(pickData(deptsRes))
        setLocations(pickData(locsRes))
      } catch {
        if (!cancelled) toast.error('Gagal memuat data master.')
      } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>Lengkapi informasi aset di bawah ini.</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <Textarea
              placeholder="Contoh: hasil pengadaan Q3-2025, ditempatkan di gudang."
              value={form.notes}
